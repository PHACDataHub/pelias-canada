...
export default function ForwardExportFiles({ apiResults }) {
	...
	useEffect(() => {
		...
		const generateInitialMetadata = () => {
			...
			// Generate checksum
			const content = apiResults.map(result => result?.result?.geocoding?.query?.text).join(",")
		...
		const features = data.map(result => {
			...
			return {
				...
				properties: {
					...
					matchType: result?.result?.features?.[0]?.properties?.match_type || "N/A",
					accuracy: result?.result?.features?.[0]?.properties?.accuracy || "N/A",
				},
			}
		})
	...
	const handleExportGeoJSON = async () => {
		...
		// Convert to GeoJSON
		const geoJSONContent = JSON.stringify(convertToGeoJSON(apiResults), null, 2)
