...
import { GcdsErrorMessage, GcdsText, GcdsHeading } from "@cdssnc/gcds-components-react"
	...
	useEffect(() => {
		if (filteredResults && filteredResults.length > 0) {
			...
			const fetchData = async () => {
				...
				setFilteredApiResults(newResults.filter(item => !singleIDs.has(item.inputID))) // Remove single-result inputIDs
				setLoading(false)
			}
