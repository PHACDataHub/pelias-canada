...
export default function reverseExportFiles({ filteredApiResults }) {
	...
	useEffect(() => {
		...
		const generateInitialMetadata = () => {
			...
			// Generate checksum
			const content = filteredApiResults.map(result => result?.result?.geocoding?.query?.text).join(",")
	...
	const convertToCSV = data => {
		...
		const rows = data.map((result, index) => {
			...
			const confidence = confidenceValue !== undefined ? confidenceValue * 100 : "N/A"
	...
	const convertToGeoJSON = data => {
		const features = data.map(result => {
			...
			const confidence = confidenceValue !== undefined ? confidenceValue * 100 : null
	...
	const handleExportGeoJSON = async () => {
		...
		// Convert to GeoJSON
		const geoJSONContent = JSON.stringify(convertToGeoJSON(filteredApiResults), null, 2)
	...
	return (
		<div>
			<GcdsButton
				...
				buttonId={t("components.reverseBulk.reverseExportFiles.exportDataCSVId")}
			...
			<GcdsButton
				...
				buttonId={t("components.reverseBulk.reverseExportFiles.exportDataGeoJSONId")}
